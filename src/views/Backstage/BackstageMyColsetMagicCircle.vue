<style scoped>
/* @import "../../Assets/css/main.css"; */
@import "../../Assets/css/backStageMyColset.css";
</style>

<template>
  <nav class="bkTitle">
    <h2>後台魔法陣管理</h2>
    <button class="add" @click="openModal">
      <div class="btnKey-M dark">
        <p>新增</p>
        <div class="icon-M addIcon">
          <div class="white-plus"></div>
        </div>
      </div>
    </button>

    <div class="searchFunction">
      <input
        class="search"
        name="search"
        type="text"
        placeholder="輸入魔法陣編號或魔法陣名稱"
        v-model="searchQuery"
      />
      <div class="icon-M searchIcon">
        <div class="dark-search"></div>
      </div>
    </div>
  </nav>
  <table class="member">
    <thead>
      <tr>
        <th class="id">編號</th>
        <th class="hairTitle">魔法陣</th>
        <!-- <th class="state">上下架</th> -->
        <th class="renewDate">新增時間</th>
        <th class="preview">預覽</th>
        <th class="delete">刪除</th>
      </tr>
    </thead>
    <tbody>
      <!-- <tr>
            <td>1</td>
            <td>flower</td>
            <td>
                <div class="state-toggle">
                    <span :class="{ active: state === '上架' }"></span>
                    <label class="switch">
                    <input type="checkbox" v-model="isOn" />
                    <span class="slider"></span>
                    </label>
                    <span :class="{ active: state === '下架' }"></span>
                </div>
            </td>
            <td>2025.03.14</td>
            <td>
                <div class="preview-container">
                    <img src=""
                    alt="預覽"
                    class="preview-image"
                    />
                </div>
            </td>
            <td>刪除</td>
          </tr> -->

      <tr v-for="(item, index) in filteredMagicCircleList" :key="index">
        <td>{{ index + 1 }}</td>
        <td>{{ item.name }}</td>
        <td>{{ item.createdAt.toDate().toLocaleDateString() }}</td>
        <td>
          <div class="preview-container">
            <img
              :src="item.imageUrl"
              alt="預覽"
              class="preview-image magicCircle"
            />
          </div>
        </td>
        <td class="deleteButton">
          <button
            @click="deleteMagicCircle(item.id, item.imageUrl)"
            class="btnKey-M dark"
          >
            刪除
          </button>
        </td>
      </tr>
    </tbody>

    <!-- 彈窗 -->
    <UploadImageLightBox
      :isVisible="isUploadImageVisible"
      @close="closeModal"
      @uploadImage="uploadImage"
    />
  </table>

  <!-- <div class="pagination">
    <button @click="prevPage" :disabled="currentPage === 1"><<</button>
    <button
      v-for="page in totalPages"
      :key="page"
      :class="{ active: currentPage === page }"
      @click="goToPage(page)"
    >
      {{ page }} /
    </button>
    <button @click="nextPage" :disabled="currentPage === totalPages">>></button>
  </div> -->
</template>

<script setup>
import { ref, onMounted, computed } from "vue";
import UploadImageLightBox from "../../components/backStage/UploadImageLightBox.vue";
import { storage, db } from "../../firebase/firebaseConfig";
import {
  ref as storageRef,
  uploadBytes,
  getDownloadURL,
  deleteObject,
} from "firebase/storage";
import {
  collection,
  addDoc,
  getDocs,
  deleteDoc,
  doc,
} from "firebase/firestore";

// 儲存魔法陣資料
const magicCircleList = ref([]);

// 搜尋框的輸入
const searchQuery = ref("");

// 根據搜尋條件過濾魔法陣
const filteredMagicCircleList = computed(() => {
  return magicCircleList.value
    .filter((magicCircle) => {
      return (
        magicCircle.name
          .toLowerCase()
          .includes(searchQuery.value.toLowerCase()) ||
        magicCircle.order.toString().includes(searchQuery.value)
      );
    })
    .sort((a, b) => a.order - b.order); // 根據 order 排序
});

// 取得魔法陣資料
const fetchMagicCircleData = async () => {
  const querySnapshot = await getDocs(collection(db, "MyClosetMagicCircles")); // 使用 "MyClosetMagicCircles" 集合
  magicCircleList.value = querySnapshot.docs.map((doc) => {
    return { id: doc.id, ...doc.data() };
  });
};

// 當頁面加載時獲取資料
onMounted(() => {
  fetchMagicCircleData();
});

// 控制彈窗顯示
const isUploadImageVisible = ref(false);

// 打開彈窗的方法
const openModal = () => {
  isUploadImageVisible.value = true;
};

// 關閉彈窗的方法
const closeModal = () => {
  isUploadImageVisible.value = false;
};

// 上傳圖片的邏輯
const uploadImage = async ({ file, imageName }) => {
  try {
    if (!imageName || !file) {
      alert("請選擇圖片並輸入名稱！");
      return; // 確保圖片名稱和檔案都存在
    }

    // 先讀取 Firestore 中的所有魔法陣資料，計算最大的 order
    const querySnapshot = await getDocs(collection(db, "MyClosetMagicCircles"));
    const currentMaxOrder = querySnapshot.docs.length; // 當前最大順序號

    const newOrder = currentMaxOrder + 1; // 新的 order 為當前資料數量（即當前資料長度）+ 1

    const storagePath = `myCloset/magicCircle/${file.name}`; // 設置魔法陣的圖片儲存路徑
    const imageRef = storageRef(storage, storagePath);

    // 上傳圖片
    await uploadBytes(imageRef, file);

    // 獲取圖片的下載 URL
    const downloadURL = await getDownloadURL(imageRef);

    // 保存到 Firestore
    await addDoc(collection(db, "MyClosetMagicCircles"), {
      name: imageName,
      fileName: file.name, // 儲存檔案的原始名稱
      imageUrl: downloadURL,
      createdAt: new Date(),
      order: newOrder,
    });

    // 重新獲取資料
    fetchMagicCircleData();

    alert("圖片上傳成功！");
    closeModal(); // 關閉彈窗
  } catch (error) {
    console.error("圖片上傳失敗：", error);
    alert("圖片上傳失敗");
  }
};

// 刪除魔法陣的邏輯
const deleteMagicCircle = async (magicCircleId, imageUrl) => {
  try {
    // 1. 刪除 Firestore 中的資料
    const magicCircleDocRef = doc(db, "MyClosetMagicCircles", magicCircleId);
    await deleteDoc(magicCircleDocRef);
    console.log("Firestore 資料已刪除");

    // 2. 刪除 Storage 中的圖片
    const imageRef = storageRef(storage, imageUrl); // 使用儲存的圖片 URL
    await deleteObject(imageRef);
    console.log("Storage 中的圖片已刪除");

    // 3. 更新頁面上的資料
    magicCircleList.value = magicCircleList.value.filter(
      (item) => item.id !== magicCircleId
    );
    alert("刪除成功！");
  } catch (error) {
    console.error("刪除操作失敗：", error);
    alert("刪除失敗！");
  }
};
</script>

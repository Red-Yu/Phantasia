    ...
    nextTick(() => {
      ...
      if (newMode === 'preview') {
        ...
        wrapperEl.__startInnerAnimation = () => {
          ...
          // 預設 textRef 初始狀態（可加 transition 效果）
      ...
      if (newMode === 'edit' || newMode === 'thumbnail') {
        ...
        tl.to(boxRefA.value, { x: 100, duration: 1.2, ease: 'power2.out' }, 0);
        tl.to(boxRefB.value, { x: -100, duration: 1.2, ease: 'power2.out' }, 0);
        tl.to(textRef.value, { y: 0, opacity: 1, duration: 1.2, ease: 'power2.out' }, '+=1');
